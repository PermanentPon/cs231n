...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dx2, dw2, db2 = affine_backward(dx0, cache2) #np.ones((scores.shape))
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            if self.use_batchnorm:
                X, cache_i = affine_bn_relu_forward(X, self.params['W' + str(i)], self.params['b' + str(i)], self.params['gamma' + str(i)], self.params['beta' + str(i)], self.bn_params[i-1])
                cache.append(cache_i)
            else:
                X, cache_i = affine_relu_forward(X, self.params['W' + str(i)], self.params['b' + str(i)])
                cache.append(cache_i)
            if self.use_dropout:
                X, cache_drop_i = dropout_forward(X, self.dropout_param)
                cache_drop.append(cache_drop_i)
        scores, cache_i = affine_forward(X, self.params['W' + str(self.num_layers)], self.params['b' + str(self.num_layers)])
        cache.append(cache_i)
